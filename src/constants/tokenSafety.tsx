...
import { KROMA_TOKENS, NATIVE_CHAIN_ID } from './tokens'
...
export function checkWarning(tokenAddress: string) {
  ...
  for (const kromatoken of KROMA_TOKENS) {
    if (kromatoken.address === tokenAddress) {
      return null
    }
  }

  if (tokenAddress === KROMA_WRAPPED_ETH_ADDRESS) {
    return null
  }
